...
RBAudio::RBAudio(const RBAudio& other)
{
...
float RBAudio::ChangeVolume(float _volume)
{
	m_fVolume += _volume;

	return m_fVolume;
}
bool RBAudio::LoadVolume(std::string _fileName)
{
	std::fstream fin;
...
float RBAudio::SetSoundPosition(float _x, float _y, float _z)
{
...
bool RBAudio::Initialize(HWND _hwnd)
{
	...
	result = LoadWaveFile((char*)"..//Resources//Sounds//sound02.wav", &m_secondaryBuffer1, &m_secondary3DBuffer1);
	if (result == false)
	{
		return false;
	}
...
bool RBAudio::InitializeDirectSound(HWND _hwnd)
{
	HRESULT result;
...
void RBAudio::UpdateListener(float _x, float _y, float _z)
{
	m_listener->SetPosition(_x,
		_y,
		_z,
		DS3D_IMMEDIATE);
...
void RBAudio::ShutdownDirectSound()
{
...
bool RBAudio::LoadWaveFile(char* _fileName, IDirectSoundBuffer8** _buffer, IDirectSound3DBuffer8** _buffer3D)
{
	int error;
	FILE* filePtr;
	size_t count;
...
void RBAudio::ShutdownWaveFile(IDirectSoundBuffer8** _buffer, IDirectSound3DBuffer8** _buffer3D)
{
	if (*_buffer3D)
	{
		(*_buffer3D)->Release();
...
bool RBAudio::PlayWaveFile()
{
	...
	result = m_secondaryBuffer1->SetVolume((LONG)m_fVolume);
	if (FAILED(result))
	{
		return false;
	}
